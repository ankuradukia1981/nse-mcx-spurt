...
def fetch_atm_combined_premium(dhan_client, security_id: int, segment: str, strike_step: int):
    """Pulls the live option chain and returns the most-active near-ATM CE+PE
    combined premium (+OI, volume, PCR, delta), or None on any failure
    (caller falls back to simulation).

    Strike selection (high-vol open robust):
      1. Theoretical ATM = round(spot / strike_step) * strike_step
      2. Consider candidates within +/-2 strike intervals of theoretical ATM
      3. Score each by liquidity = (CE_OI + PE_OI) + 2*(CE_vol + PE_vol)
      4. Pick the highest-scoring strike (fallback: pure nearest)
    """
    try:
        ...
        for k in oc.keys():
            ...
            candidates.append((k, strike, liquidity, distance, ce, pe, ce_oi, pe_oi, ce_vol, pe_vol))
        ...
        if not candidates:
            ...
            _, best_strike, _, _, ce, pe, ce_oi, pe_oi, ce_vol, pe_vol = candidates[0]

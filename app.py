"""
NSE / MCX Combined-Premium Terminal — Single-Panel Live Edition (v5)
---------------------------------------------------------------------
Single-page Streamlit dashboard: header + ticker strip + controls,
Alerts/Stats pair, day-persistent spike watchlist + full-page
Combined Premium + Volume chart per symbol.

Key rules (v5):
  - Expiry  = nearest active expiry only.
  - Strike  = most-active near-ATM (liquidity-weighted OI+Volume within
              +/-2 strikes of theoretical ATM) — robust at the open.
  - Spurt % = (current comb. prem - first post-open baseline) / baseline.
              Calculation starts only after the opening bell.
  - Watchlist is session-scoped: any symbol that crosses +/-threshold stays
    locked on the list until the next trading day, even if %chg falls back.
  - One row per symbol (single expiry + single strike).
  - Chart shows Combined Premium (primary) + Volume (or OI fallback).

Requires .env / Streamlit Secrets:
    DHAN_CLIENT_ID=...
    DHAN_ACCESS_TOKEN=...
Missing/invalid creds -> graceful SIM fallback (never crashes).
...
_defaults = {
    ...
    "view": "dashboard",
    "active_sym": "NIFTY",
    # Session-scoped: symbols that crossed +/-threshold at least once today stay
    # on the watchlist until the next trading day even if %chg falls back.
    "spiked_today": set(),
    "session_date": None,   # YYYY-MM-DD used to detect day rollover
    "baselines": {},        # first valid combined_premium after open per symbol
}
...
# Day rollover: clear session-scoped spike lock & baselines when a new IST day starts
_today = datetime.now().strftime("%Y-%m-%d")
if st.session_state.session_date != _today:
    st.session_state.session_date = _today
...
def run_universe_tick(threshold: float):
    ...
    Spurt % is calculated only after the opening bell (MARKET_OPEN).
    Baseline = first valid Combined Premium recorded after open for that symbol.
    Once a symbol crosses +/-threshold it is locked into spiked_today for the
    rest of the trading day (watchlist stays persistent).
    """
    ...
    for sym, cfg in ALL_INSTRUMENTS.items():
        ...
        # Establish / reuse post-open baseline
        if MARKET_OPEN:
            if sym not in st.session_state.baselines and reading.get("combined_premium"):
                st.session_state.baselines[sym] = reading["combined_premium"]
            baseline = st.session_state.baselines.get(sym) or reading["combined_premium"]
        else:
            # Pre-open: no spurt calc
            baseline = reading["combined_premium"]

        pct_chg = ((reading["combined_premium"] - baseline) / baseline) * 100 if baseline else 0.0
        is_spike = MARKET_OPEN and abs(pct_chg) >= threshold

        if is_spike:
            st.session_state.spiked_today.add(sym)

        is_spike_today = sym in st.session_state.spiked_today
        ...
        if is_spike:
            already_recent = any(
                a["symbol"] == sym and (datetime.now() - a["ts"]).seconds < 25
...
for sym, cfg in ALL_INSTRUMENTS.items():
    ...
    if not r:
        continue
    spiked = r.get("is_spike_today") or r.get("is_spike")
    cls = "tick-spike" if spiked else ("tick-up" if r["pct_chg"] >= 0 else "tick-down")
    arrow = "\u25b2" if r["pct_chg"] >= 0 else "\u25bc"
    ticker_html += (
        f'<span><b>{cfg["label"]}</b> \u20b9{r["combined_premium"]:.1f} '
...
if st.session_state.view == "chart" and st.session_state.active_sym in ALL_INSTRUMENTS:
    ...
    with top_l:
        st.markdown(f"### {inst['label']} — Combined Premium + Volume")
    with top_r:
    ...
    if hist:
        ...
        if "total_vol" in df.columns and df["total_vol"].sum() > 0:
            fig.add_trace(go.Bar(
                x=df["time"], y=df["total_vol"], name="Volume (CE+PE)",
                marker_color="rgba(6,182,212,0.35)", yaxis="y3",
            ))
            vol_label = "Volume"
        else:
            ...
            if total_oi is not None:
                fig.add_trace(go.Bar(
                    x=df["time"], y=total_oi, name="Total OI (CE+PE)",
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=True, gridcolor="#1e293b", nticks=12, title="Session time"),
        ...
        st.caption(
            f"Expiry: {latest.get('expiry', '-')} \u2022 Strike: {latest.get('atm_strike', '-')} "
            f"\u2022 CE: \u20b9{latest.get('ce_ltp', 0):.2f} \u2022 PE: \u20b9{latest.get('pe_ltp', 0):.2f} "
            f"\u2022 Comb: \u20b9{latest.get('combined_premium', 0):.2f} "
            f"\u2022 Source: {latest.get('source', '-')} \u2022 {vol_label} shown"
        )
    else:
...
# ==========================================================================
    ...
    # ---- WATCHLIST: symbols that crossed +/-threshold at least once today
    # (persistent until next trading day). Sorted by absolute %chg descending.
    spiking = [
        (s, r) for s, r in current_records.items()
        if r.get("is_spike_today") or abs(r.get("pct_chg", 0)) >= threshold
    ]
    spiking.sort(key=lambda x: abs(x[1]["pct_chg"]), reverse=True)

    st.markdown(
        f'<div class="wl-header">F&O Watchlist — Combined Premium Monitor '
        f'<span class="wl-badge">DAY SPIKES \u2265{threshold:.1f}%</span>'
        f'<span class="wl-count">{len(spiking)} of {len(ALL_INSTRUMENTS)} instruments '
        f'(locked for session)</span></div>',
        unsafe_allow_html=True,
    )

    if not spiking:
        st.markdown(
            f'<div class="empty-state">No instrument has crossed the '
            f'\u00b1{threshold:.1f}% threshold yet today '
            f'(spurt calc starts post opening bell).</div>',
            unsafe_allow_html=True,
        )
    else:
        # Required order: Symbol | Expiry | Strike | CE | PE | Comb.Prem | %Chg | Status | Chart
        hdr = st.columns([1.4, 1.2, 1.0, 0.9, 0.9, 1.1, 0.9, 0.9, 0.8])
        for c, label in zip(hdr, [
            "Symbol", "Expiry", "Strike", "CE", "PE",
            "Comb.Prem", "%Chg", "Status", "",
        ]):
            c.markdown(
                f'<span style="color:var(--text-dim);font-size:10px;text-transform:uppercase">{label}</span>',
                unsafe_allow_html=True,
            )
        ...
        for sym, r in spiking:
            ...
            status = "SPIKE" if r.get("is_spike_today") else "—"
            cols = st.columns([1.4, 1.2, 1.0, 0.9, 0.9, 1.1, 0.9, 0.9, 0.8])
            cols[0].markdown(
                f"**{label}**<br><span style='color:var(--text-dim);font-size:10px'>"
                f"Spot {r['spot']:,.1f}</span>",
                unsafe_allow_html=True,
            )
            cols[1].markdown(f"{r.get('expiry', '-')}")
            cols[2].markdown(f"{r['atm_strike']:,.0f}")
            cols[3].markdown(f"{r['ce_ltp']:.2f}")
            cols[4].markdown(f"{r['pe_ltp']:.2f}")
            cols[5].markdown(f"\u20b9{r['combined_premium']:.2f}")
            cols[6].markdown(
                f'<span class="{pct_cls}">{r["pct_chg"]:+.2f}%</span>',
                unsafe_allow_html=True,
            )
            if status == "SPIKE":
                cols[7].markdown('<span class="tag-spike">SPIKE</span>', unsafe_allow_html=True)
            else:
                cols[7].markdown("—")
            if cols[8].button("Chart", key=f"chart_{sym}"):
                st.session_state.active_sym = sym
                st.session_state.view = "chart"
                st.rerun()

    st.markdown(
        '<div class="foot">Combined Premium = nearest-expiry most-active ATM Call LTP + Put LTP. '
        'Spurt % uses first post-open premium as baseline. Symbols that cross the threshold '
        'stay on the day watchlist until the next trading session. Not investment advice.</div>',
        unsafe_allow_html=True,
    )
